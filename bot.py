...
try:
    from config import API_ID, API_HASH, BOT_TOKEN, CHANNEL_IDENTIFIER, ADMIN_ID
except ImportError:
...
REGISTRATION_LINK = "https://lb-aff.com/L?tag=d_4514392m_66803c_apk1&site=4514392&ad=66803"
CONTACT_USERNAME = "@bet_store3"
TUTORIAL_LINK = "https://t.me/c/1672479948/5097"
...
في الأول لازم تسجّل في LineBet بحساب جديد بالبرومو كود او الرمز الترويجي الخاص بينا:   BB33

رابط التسجيل : 🔥🔥 https://lb-aff.com/L?tag=d_4514392m_22611c_site&site=4514392&ad=22611&r=registration

رابط تحميل التطبيق : https://lb-aff.com/L?tag=d_4514392m_66803c_apk1&site=4514392&ad=66803

بعدها بتحط إيداع 145 جنيه او اكتر، وبعدها بتستعمل إشاراتي الـ VIP عشان تكسب!

🚀 تابع قناتي وشوف بنفسك! 👇
https://t.me/+pRYIKFjCY6BlZmZk

نصيبي هو 10% من أرباحك في الشهر!
خلينا نكون صريحين مع بعض 🤝😉

اول ما تعمل ارباح صورهالي و ابعتها هنا نشارك بيها الناس مع بعض ❤️
كلمني علي : @bet_store3
"""
PLAN_ONE_TEXT = """
...
⚠️ معلومة: الكود الترويجي BB33 متاح لأول 50 شخص فقط كل يوم
...
✅ سجل واشحن الـ 145 جنيه
✅ جرب أول 3 إشارات
✅ لو ما كسبتش في واحدة منهم...
💰 أنا شخصياً أرجعلك الـ 145 جنيه كاملة!
...
async def apple_game_loop():
    ...
    while True:
        try:
            ...
            success = await send_text_safe(message_host, user_target_channel, GAME_INTRO_TEXT)
            if success:
            ...
            try:
                await message_host.send_message(user_target_channel, game_text, buttons=buttons)
            ...
            success = await send_text_safe(message_host, user_target_channel, GAME_CONGRATS_TEXT)
            if success:
...
async def send_plan_one():
    try:
        await send_photos_with_caption(message_host, user_target_channel, PLAN_ONE_IMAGES, PLAN_ONE_TEXT)
...
async def send_plan_two():
    try:
        await send_photos_with_caption(message_host, user_target_channel, PLAN_TWO_IMAGES, PLAN_TWO_TEXT)
...
async def send_plan_three():
    try:
        await send_photo_with_caption(message_host, user_target_channel, PLAN_THREE_IMAGE, PLAN_THREE_TEXT)
...
async def send_welcome_to_user(user_or_id, user_name: str = "عضو جديد"):
    """إرسال رسالة ترحيب للعضو الجديد باستخدام user entity أو user_id"""
    # استخراج الـ integer id لاستخدامه في الـ locks والـ state
    user_id = getattr(user_or_id, 'id', None) or int(user_or_id)
    target = user_or_id  # قد يكون entity كامل أو مجرد id
    ...
    async with lock:
        try:
            # محاولة إرسال الرسالة مباشرة باستخدام target (entity أو id) مع الأزرار
            await sender.send_message(target, WELCOME_MSG, parse_mode="md", buttons=create_action_buttons())
...
📅 التاريخ: {datetime.now(TIMEZONE).strftime('%Y-%m-%d')}"""
        ...
        async def setchannel_handler(event):
            ...
            try:
                ...
                if channel_id.isdigit() or channel_id.startswith('-'):
                    new_channel = await bot_client.get_entity(int(channel_id))
                else:
        ...
        # Handler لطلبات الانضمام مباشرة على البوت
        @bot_client.on(events.Raw(types=[UpdateBotChatInviteRequester]))
        async def handle_bot_join_request(event):
            try:
                ...
                # التحقق من أن الطلب من القناة المستهدفة
                event_channel_id = getattr(peer, 'channel_id', None)
                target_channel_id = getattr(user_target_channel, 'id', None) if user_target_channel else None
                if target_channel_id and event_channel_id != target_channel_id:
                    return

                log.info(f"👥 [BOT] Join request from user {user_id}")

                user_name = "عضو جديد"

                # محاولة جلب اسم المستخدم من الـ cache الداخلي لـ Telethon
                try:
                    cached = await bot_client.get_entity(InputUser(user_id=user_id, access_hash=0))
                    user_name = getattr(cached, 'first_name', 'عضو جديد') or 'عضو جديد'
                    log.info(f"✅ Got name from cache: {user_name}")
                except Exception:
                    pass

                # الموافقة على الطلب باستخدام HideChatJoinRequestRequest مع access_hash=0
                # (البوت يملك صلاحية ضمنية لما يستقبل UpdateBotChatInviteRequester)
                try:
                    input_user = InputUser(user_id=user_id, access_hash=0)
                    await bot_client(HideChatJoinRequestRequest(peer=peer, user_id=input_user, approved=True))
                    log.info(f"✅ [BOT] Approved join request for user {user_id}")
                except FloodWaitError as fe:
                    log.warning(f"FloodWait during bot approve: {fe.seconds}s")
                    await safe_sleep(fe.seconds + 1)
                except Exception as e:
                    log.error(f"❌ [BOT] Error approving join request: {e}")

                # إرسال رسالة الترحيب
                if user_id not in _recently_processed_joins:
                    _recently_processed_joins[user_id] = time.time()
                    await send_welcome_to_user(user_id, user_name)
            except Exception as e:
                log.error(f"❌ [BOT] Error in handle_bot_join_request: {e}")
...
async def find_channel_by_name(channel_name: str) -> Optional[object]:
    """البحث عن القناة بالاسم من dialogs البوت"""
    try:
        async for dialog in bot_client.iter_dialogs():
            if dialog.is_channel:
...
async def find_target_channel_once() -> Optional[object]:
    """البحث عن القناة المستهدفة بالمعرف أو الاسم أو من قائمة محادثات البوت"""
    ...
    if str(CHANNEL_IDENTIFIER).lstrip('-').isdigit():
        raw = str(CHANNEL_IDENTIFIER).lstrip('-')
        if raw.startswith('100'):
            raw = raw[3:]
        target_id = int(raw)

    # محاولة get_entity مباشرة
    try:
        if str(CHANNEL_IDENTIFIER).isdigit() or str(CHANNEL_IDENTIFIER).startswith('-'):
            ch = await bot_client.get_entity(int(CHANNEL_IDENTIFIER))
            log.info(f"✅ Found channel via bot client: {getattr(ch,'title','')}")
            return ch
    except Exception as e:
        log.warning(f"⚠️ [bot] Could not find channel by ID: {e}")

    # البحث في قائمة المحادثات
    try:
        log.info(f"🔍 [bot] Searching through dialogs...")
        async for dialog in bot_client.iter_dialogs():
            ch = dialog.entity
            ch_id = getattr(ch, 'id', None)
            ch_title = getattr(ch, 'title', '')
            if target_id and ch_id == target_id:
                log.info(f"✅ Found channel in bot dialogs: {ch_title}")
                return ch
            if not target_id and CHANNEL_IDENTIFIER and CHANNEL_IDENTIFIER.lower() in ch_title.lower():
                log.info(f"✅ Found channel by name in bot dialogs: {ch_title}")
                return ch
    except Exception as e:
        log.warning(f"⚠️ [bot] Error searching dialogs: {e}")
...
async def main():
    ...
    # تشغيل البوت
    if not bot_client or not BOT_TOKEN:
        log.critical("❌ BOT_TOKEN غير مضبوط. الرجاء ضبطه في config.py أو متغيرات البيئة.")
        return
    ...
    # إيجاد القناة المستهدفة
    log.info("🔍 Searching for target channel...")
    user_target_channel = await find_target_channel_once()
    if not user_target_channel:
        log.critical(f"❌ Could not find target channel '{CHANNEL_IDENTIFIER}'. Make sure the bot is admin in the channel.")
        try:
    ...
    message_host = bot_client
    log.info("🤖 Message host: bot account.")
